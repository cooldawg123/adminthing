-- tdb  source leak ez pz 

if game.PlaceId == 6441847031 then
    local player = game.Players.LocalPlayer

    -- Normal player functionality
    print("Welcome, " .. player.Name .. "! You are a user of my script.")
    
    -- You can add more features for normal players here
    local OrionLib = loadstring(game:HttpGet('https://raw.githubusercontent.com/shlexware/Orion/main/source'))()
    local Window = OrionLib:MakeWindow({
        Name = "TDB-E Team",
        HidePremium = false,
        IntroText = "Welcome to TDB-E Team Script!",
        SaveConfig = true,
...
Name = "Message Leader!",
Content = "Main reason I use script is to kill Exploits abuse, and to socialize I don't consider anyone as a rival, but if I see a Exploit from another clan or the same clan, killing innocents, I will focus on him",
...
OrionLib:MakeNotification({
Name = "Notification!",
Content = "KingDarkness hope you good luck",
Image = "rbxassetid://4483345998",
Time = 5
})
...
Callback = function()
      print("button pressed")
  loadstring(game:HttpGet("https://pastebin.com/raw/GsyGYdbz"))()
  end    
})

FarmTab:AddButton({
Name = "Fly Gui (click 1 times only)",
Callback = function()
      print("button pressed")
...
local player = game.Players.LocalPlayer
    local adminUserIds = {661316734, 1239486533} -- Replace with actual admin User IDs
    ...
    if isAdmin() then
        -- Admin Panel
        local AdminWindow = OrionLib:MakeWindow({
            Name = "Admin Panel | TDB-E Team",
            HidePremium = false,
            IntroText = "Welcome to TDB-E Team Admin Panel",
            SaveConfig = true,
            ConfigFolder = "OrionTest"
        })

        -- Create the Home tab
        local HomeTab = AdminWindow:MakeTab({
            Name = "Home",
            Icon = "rbxassetid://4483345998",
            PremiumOnly = false
        })

        HomeTab:AddLabel("Welcome Home, Admin!")

        -- Create the Kick Player tab
        local FarmTab = AdminWindow:MakeTab({
            Name = "Kick Player",
            Icon = "rbxassetid://4483345998",
            PremiumOnly = false
        })

        FarmTab:AddLabel("Kick Player")

        -- Input box for player name to search
        local playerToKickName = ""
        ...
        Window:MakeNotification({
            Name = "Welcome!",
            Content = "Welcome, " .. player.Name .. "! You are a user of this script.",
            Time = 5
        })

        -- Notify admins when a normal player opens the script
        for _, id in ipairs(adminUserIds) do
            local adminPlayer = game.Players:GetPlayerByUserId(id)
            if adminPlayer then
                adminPlayer:SendNotification({
                    Title = "Normal Player Active",
                    Text = player.Name .. " has opened the normal script.",
                    Duration = 5
                })
            end
        end

        -- Additional normal player features can be added here
        Window:MakeTab({
            Name = "User Features",
            Icon = "rbxassetid://4483345998",
            PremiumOnly = false
        }):AddLabel("Enjoy your time in the game!")
...
FarmTab:AddLabel("Tab 3 is for the Premium Member only")
...
FarmTab:AddButton({
    ...
    Callback = function()
        local allowedPlayers = {
            "TDB_KingDarkness", -- Replace with the first allowed player's name
            "TDB_big1darkness1" -- Replace with the second allowed player's name
        }

        -- Check if the current player's name is in the allowed players list
        ...
        if isAllowed then
            if not buttonPressed then -- Check if the button has already been pressed
                buttonPressed = true -- Set the flag to true
                print("button pressed by " .. playerName)
...
PrivateProperties.Text = "{SPY BY TDB} { "..(Config.enabled and "EN" or "DIS").."ABLED}"
StarterGui:SetCore("ChatMakeSystemMessage", PrivateProperties)
local chatFrame = player.PlayerGui.Chat.Frame
chatFrame.ChatChannelParentFrame.Visible = true
chatFrame.ChatBarParentFrame.Position = chatFrame.ChatChannelParentFrame.Position+UDim2.new(UDim.new(),chatFrame.ChatChannelParentFrame.Size.Y)
            else
                print("This button can only be pressed once.")
            end
        else
            print("You are not allowed to press this button.")
        end
    end    
})
